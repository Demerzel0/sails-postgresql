  ...
  Object.keys(options).forEach(function(key) {
    switch(key.toLowerCase()) {
      case 'or':
    	...
    	// Add And statement if not first where item
    	if (!first) {
    		self._query += ' AND ';
    		first = true; // Next or is the first of the parenthesis
    	}

    	// Use parenthesis between or statements
    	self._query += ' (';
            ...
            switch(key) {
              case 'and':
                Object.keys(statement[key]).forEach(function(attribute) {
                  operators.and(attribute, statement[key][attribute], ' OR ', first);
                });
                break;

              case 'like':
                Object.keys(statement[key]).forEach(function(attribute) {
                  operators.like(attribute, key, statement, ' OR ', first);
                });
                break;
              ...
              default:
                ...
                operators.and(key, statement[key], ' OR ', first);
                break;
            }
          });
      ...
      case 'like':
        Object.keys(options[key]).forEach(function(parent) {
          operators.like(parent, key, options, first);
        });
        break;
      ...
      default:
        ...
        if(options[key] instanceof Array) {
          operators["in"](key, options[key], null, first);
          break;
        }

        // `AND`
        operators.and(key, options[key], null, first);
        break;
    }
    first = false;
...
Query.prototype.operators = function() {
  ...
  var sql = {
    and: function(key, options, comparator, first) {
    ...
    like: function(parent, key, options, comparator, first) {
      var caseSensitive = true;
    ...
    "in": function(key, options, first) {
      ...
      if(!caseSensitivity){key = 'LOWER("' + key + '")';} else {key = '"' + key + '"'; // for case sensitive camelCase columns
}
...
function prepareCriterion(key, value) {
  ...
  // Bump paramCount
  this._paramCount =  this._values.length +1;
...
Query.prototype.cast = function(values) {
  ...
  Object.keys(values).forEach(function(key) {
    ...
    if(type === 'array') {
      try {
	if( Object.prototype.toString.call( _values[key] ) === '[object Array]' ) {
            // if already a real array do nothing
         return;
        } else {
